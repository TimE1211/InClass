...
//    addChild(invader)
  ...
  func fireShipBullets()
  {
    ...
    if existingBullet == nil
    {
      if let ship = childNode(withName: kShipName)
      {
        ...
        bullet.position = CGPoint(x: ship.position.x, y: ship.position.y + ship.frame.height - bullet.frame.size.height/2)
  ...
  func fireInvaderBulletsForUpdate(_ currentTime: TimeInterval)
  {
    ...
    if existingBullet == nil
    {
      ...
      {
        let randomInvaderIndex = Int(arc4random_uniform(UInt32(allInvaders.count)))
        let invader = allInvaders[randomInvaderIndex]
